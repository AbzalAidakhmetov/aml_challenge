...
# Set an array of SEEDS
SEEDS = [42, 322, 323, 10000, 100001, 100002]
...
def run_for_seed(seed: int):
    global RANDOM_SEED, MODEL_PATH, MODEL_PATH_MRR, SUBMISSION_PATH

    # 1) Set seed-dependent paths
    RANDOM_SEED = seed
    MODEL_PATH = f"models/mlp_seed{seed}_best_loss.pth"
    MODEL_PATH_MRR = f"models/mlp_seed{seed}_best_mrr.pth"

    if seed != 42: 
        SUBMISSION_PATH = f"submission_seed{seed}_mrr.csv"
    else:
        # since we already have seed 42 on old dataset
        # for simplicity we renamed it to 404
        SUBMISSION_PATH = f"submission_seed{404}_mrr.csv"   


    print(f"\n===== RUNNING SEED {seed} =====")
    print(f"MODEL_PATH       = {MODEL_PATH}")
    print(f"MODEL_PATH_MRR   = {MODEL_PATH_MRR}")
    ...
    # 4) Train/val split based on this seed
    ...
    # 7) Evaluate + load best checkpoint for this seed
    ...
    print(f"   Final validation MRR (seed={seed}): {final_mrr:.6f}")

    # 8) Generate submission for this seed
    print("\n6. Generating submission from best model for this seed...")
    ...
    print(f"✓ [seed={seed}] Best-loss model saved to: {MODEL_PATH}")
    print(f"✓ [seed={seed}] Best-MRR model saved to: {MODEL_PATH_MRR}")
    print(f"✓ [seed={seed}] Submission saved to: {SUBMISSION_PATH}")
