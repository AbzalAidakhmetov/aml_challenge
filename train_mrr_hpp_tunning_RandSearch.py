...
# Controls the maximum amount of GPU memory (in bytes) a single Manhattan
# similarity block is allowed to allocate. The chunking logic will split the
# pairwise computation accordingly to avoid NxMxD tensors that do not fit in
# memory (especially when using torch.cdist with p=1).
MANHATTAN_MAX_BLOCK_BYTES = 512 * 1024 * 1024  # 512 MB per block
...
def _chunked_manhattan_similarity(
    a: torch.Tensor,
    b: torch.Tensor,
    max_block_bytes: int = MANHATTAN_MAX_BLOCK_BYTES,
) -> torch.Tensor:
    """
    Compute -L1 distance (Manhattan similarity) between all rows of `a` and `b`
    without instantiating the full [N, M, D] tensor that torch.cdist would
    allocate. This prevents CUDA OOM when both batch size and embedding dim are
    large.
    """
    if a.dim() != 2 or b.dim() != 2:
        raise ValueError("Expected 2D tensors for chunked Manhattan similarity")
    n, m = a.size(0), b.size(0)
    if n == 0 or m == 0:
        return a.new_empty((n, m))

    dtype_bytes = a.element_size()
    feature_dim = a.size(1)
    max_pairs = max(
        1, max_block_bytes // max(1, dtype_bytes * max(1, feature_dim))
    )
    # Use a square block for simplicity; cap by actual dimensions.
    block_size = max(1, int(math.sqrt(max_pairs)))
    block_rows = max(1, min(block_size, n))
    block_cols = max(1, min(block_size, m))

    if block_rows == n and block_cols == m:
        return -torch.cdist(a, b, p=1)

    logits = a.new_empty((n, m))
    for row_start in range(0, n, block_rows):
        row_end = min(row_start + block_rows, n)
        a_chunk = a[row_start:row_end]
        for col_start in range(0, m, block_cols):
            col_end = min(col_start + block_cols, m)
            b_chunk = b[col_start:col_end]
            block = torch.cdist(a_chunk, b_chunk, p=1)
            logits[row_start:row_end, col_start:col_end] = -block
    return logits
...
def compute_pairwise_logits(
    ...
    if metric == "cosine":
        ...
        logits = _chunked_manhattan_similarity(a, b)
...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        print(
            ...
            f"sim_metric = {SIMILARITY_METRIC_TRAIN}"
...
# Seed dedicated to hyperparameter sampling. Leave as None to let sampling use
# system entropy so it remains different from the training RANDOM_SEED.
HP_SAMPLING_SEED: int | None = None
...
HP_SPACE = {
    ...
    "epochs": [50],
    "similarity_metric_train": ["cosine"],  # NEW
...
def sample_hparams(seen_configs: set, rng: random.Random) -> dict:
    """
    Randomly sample a configuration from HP_SPACE, avoiding duplicates
    until all combinations are exhausted.
    """
    # Total number of unique combos (for safety)
    total_combos = 1
    for values in HP_SPACE.values():
        total_combos *= len(values)

    # If we've already seen all combos, just return a random one (no uniqueness)
    if len(seen_configs) >= total_combos:
        return {k: rng.choice(vs) for k, vs in HP_SPACE.items()}

    while True:
        config = {k: rng.choice(vs) for k, vs in HP_SPACE.items()}
...
def hyperparam_search():
    ...
    hp_rng.seed(HP_SAMPLING_SEED)

    results = []
    global_best_mrr = -1.0
    global_best_result = None
    print(f"\nRunning random search with {HP_NUM_TRIALS} trials...\n")

    for trial in range(HP_NUM_TRIALS):
        config = sample_hparams(seen_configs, hp_rng)
